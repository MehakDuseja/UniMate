...
from agent.retriever import get_candidate_universities, get_university_options
...
_EDUCATION_OPTIONS = ["", "Intermediate", "A-Levels", "FSc Pre-Engineering", "FSc Pre-Medical", "ICS", "O-Levels", "Bachelor's", "Other"]
_BOARD_OPTIONS = ["", "Federal Board", "Sindh Board", "Punjab Board", "Cambridge (A-Levels)", "Aga Khan Board", "Other"]
...
def _profile_matches(profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if not candidates:
            return []
        return rank_candidates(profile, candidates, priority_focus=profile.get("priority_focus"), limit=5)
    except Exception:
        return []


def _read_draft(prof: dict[str, Any], university_labels: list[str]) -> dict[str, Any]:
    ...
    age_val = st.session_state.get("pf_age", prof.get("age") or 0)

    raw = {
        "name": st.session_state.get("pf_name", prof.get("name", "")),
        "email": st.session_state.get("pf_email", prof.get("email", "")),
        "phone": st.session_state.get("pf_phone", prof.get("phone", "")),
        "age": age_val or None,
        ...
        "board": st.session_state.get("pf_board", prof.get("board", "")),
        "academic_percentage": st.session_state.get("pf_academic", prof.get("academic_percentage") or 0.0),
        "target_universities": st.session_state.get("pf_universities", [x for x in (prof.get("target_universities") or []) if x in university_labels]),
...
def render_profile_form(get_graph: Callable[[], Any]) -> None:
    ...
    university_options = get_university_options()[1:]
    university_labels = [o["label"] for o in university_options]

    header_col, progress_col = st.columns([3, 1])
    draft = _read_draft(prof, university_labels)
    ...
    with c1:
        ...
        st.text_input("Phone number", value=prof.get("phone", ""), placeholder="+92 300 1234567", key="pf_phone")
        st.text_input("Your city", value=prof.get("student_city", ""), key="pf_city")
    with c2:
        st.text_input("Email", value=prof.get("email", ""), key="pf_email")
        st.number_input("Age", min_value=0, max_value=100, value=int(prof.get("age") or 0), key="pf_age")
    ...
    with ac3:
        st.selectbox("Board", _BOARD_OPTIONS, index=_BOARD_OPTIONS.index(prof.get("board", "")) if prof.get("board") in _BOARD_OPTIONS else 0, key="pf_board")
    fc1, fc2 = st.columns(2)
    with fc1:
        st.number_input("Academic percentage", min_value=0.0, max_value=100.0, value=float(prof.get("academic_percentage") or 0.0), step=0.1, key="pf_academic", help="Enter 0 if you have not received a percentage yet.")
    with fc2:
    ...
    st.multiselect(
        "Target universities",
        university_labels,
        default=[x for x in (prof.get("target_universities") or []) if x in university_labels],
        key="pf_universities",
    )
    ...
    with st.container(key="profile_save_bar"):
